...
namespace Laborator5
{
    class Program
    {
        ...
        public static void addSelfReferences()
        {
            Model1 modelSelfRefrences = new Model1();
          //  modelSelfRefrences.SelfReferences.Add(new SelfReference() { Name = "Bogdan" });
           // modelSelfRefrences.SaveChanges();
            var allSelfReferences = modelSelfRefrences.Set<SelfReference>().ToList();
            for (var i = 0; i < allSelfReferences.Count; i++)
            {
                Console.WriteLine(allSelfReferences[i].Name);
            }
        ...
        public static void addProduct()
        {
            using (var context = new Model2()){
                ...
                context.Products.Add(product); product = new Product { SKU = 186, Description = "Range Field Pack", Price = 98.97M, ImageURL = "/noimage.jp" }; 
                context.Products.Add(product); product = new Product { SKU = 202, Description = "Small Deployment Back Pack", Price = 29.97M, ImageURL = "/pack202.jpg" }; 
                context.Products.Add(product); 
                context.SaveChanges();
            }
            using (var context = new Model2())
        ...
        public static void addImage()
        {
...

            {
                ...
                context.Photograph.Add(photo); context.SaveChanges();
            }

            using (var context = new Model3())
            {
                foreach (var photo in context.Photograph)
                {
                    Console.WriteLine("Photo: {0}, ThumbnailSize {1} bytes",
                    photo.Title, photo.ThumbnailBits.Length);
                    // explicitly load the "expensive" entity,
                    context.Entry(photo).Reference(p => p.PhotographFullImage).Load();Console.WriteLine("Full Image Size: {0} bytes",photo.PhotographFullImage.HighResolutionBits.Length);
                }
            }
          }
        public static void addBusiness()
        {
            using (var context = new Model4())
